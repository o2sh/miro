...
impl EglWrapper {
    ...
    pub fn choose_config(
        ...
        if unsafe { self.egl.GetConfigs(display, std::ptr::null_mut(), 0, &mut num_configs) } != 1 {
...
unsafe impl glium::backend::Backend for GlState {
    ...
    fn get_framebuffer_dimensions(&self) -> (u32, u32) {
        ...
        unsafe {
            self.egl.egl.QuerySurface(self.display, self.surface, ffi::WIDTH as i32, &mut width);
        }
        unsafe {
            self.egl.egl.QuerySurface(self.display, self.surface, ffi::HEIGHT as i32, &mut height);
        }
    ...
    unsafe fn make_current(&self) {
        self.egl.egl.MakeCurrent(self.display, self.surface, self.surface, self.context);
    }
}
