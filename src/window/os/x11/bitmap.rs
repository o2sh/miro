...
impl Context {
    pub fn new(conn: &Rc<Connection>, d: &dyn Drawable) -> Context {
        ...
        Context { gc_id, conn: Rc::clone(conn), drawable }
    }
...
impl ShmId {
    ...
    fn new(size: usize) -> Fallible<ShmId> {
        ...
        if id == -1 {
            bail!("shmget failed for {} bytes: {:?}", size, std::io::Error::last_os_error());
        }
        Ok(ShmId { id })
    }
    ...
    fn attach(&self) -> Fallible<ShmData> {
        ...
        if data as usize == !0 {
            bail!("shmat failed: {:?} {}", data, std::io::Error::last_os_error());
        }
...
impl ShmImage {
    pub fn new(
        ...
        Ok(ShmImage { data, seg_id, draw_id, conn: Rc::clone(conn), width, height })
    }
}
