...
mod test {
    ...
    fn basic() {
        ...
        assert_eq!(parse('m', &[1], "\x1b[1m"), vec![CSI::Sgr(Sgr::Intensity(Intensity::Bold))]);
        assert_eq!(
            parse('m', &[1, 3], "\x1b[1m\x1b[3m"),
            vec![CSI::Sgr(Sgr::Intensity(Intensity::Bold)), CSI::Sgr(Sgr::Italic(true)),]
        );
    ...
    fn color() {
        ...
        assert_eq!(
            ...
            vec![CSI::Sgr(Sgr::Foreground(ColorSpec::TrueColor(RgbColor::new(255, 255, 255),)))]
        );
        assert_eq!(
    ...
    fn edit() {
        assert_eq!(
            parse('J', &[], "\x1b[J"),
            vec![CSI::Edit(Edit::EraseInDisplay(EraseInDisplay::EraseToEndOfDisplay,))]
        );
        assert_eq!(
            parse('J', &[0], "\x1b[J"),
            vec![CSI::Edit(Edit::EraseInDisplay(EraseInDisplay::EraseToEndOfDisplay,))]
        ...
        assert_eq!(
            parse('J', &[1], "\x1b[1J"),
            vec![CSI::Edit(Edit::EraseInDisplay(EraseInDisplay::EraseToStartOfDisplay,))]
        );
    }

    #[test]
    fn cursor() {
        assert_eq!(parse('C', &[], "\x1b[C"), vec![CSI::Cursor(Cursor::Right(1))]);
        // check that 0 is treated as 1
        assert_eq!(parse('C', &[0], "\x1b[C"), vec![CSI::Cursor(Cursor::Right(1))]);
        assert_eq!(parse('C', &[1], "\x1b[C"), vec![CSI::Cursor(Cursor::Right(1))]);
        assert_eq!(parse('C', &[4], "\x1b[4C"), vec![CSI::Cursor(Cursor::Right(4))]);
    }
    ...
    fn decset() {
        assert_eq!(
            ...
            vec![CSI::Mode(Mode::SetDecPrivateMode(DecPrivateMode::Unspecified(23434),))]
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            vec![CSI::Mode(Mode::RestoreDecPrivateMode(DecPrivateMode::Code(
    ...
    fn device_attr() {
        assert_eq!(
            parse_int('c', &[63, 1, 2, 4, 6, 9, 15, 22], b'?', "\x1b[?63;1;2;4;6;9;15;22c"),
            vec![CSI::Device(Box::new(Device::DeviceAttributes(DeviceAttributes::Vt320(
                DeviceAttributeFlags::new(vec![
