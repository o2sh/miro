...
impl Connection {
    pub fn new() -> Result<Connection> {
        ...
        let first_config = *configs
            .first()
            .ok_or(failure::err_msg("no compatible EGL configuration was found"))?;
...
impl<'a> Window<'a> {
    ...
    pub fn new(conn: &Connection, width: u16, height: u16) -> Result<Window> {
        ...
        conn.egl_display.make_current(&surface, &surface, &egl_context).map_err(egli_err)?;

        gl::load_with(|s| unsafe { mem::transmute(egli::egl::get_proc_address(s)) });

        let gl_state =
            Rc::new(GlState { display: Rc::clone(&conn.egl_display), egl_context, surface });
        ...
        Ok(Window { conn, window_id, gl: gl_state, glium_context })
    }
    ...
    pub fn draw(&self) -> glium::Frame {
        glium::Frame::new(self.glium_context.clone(), self.gl.get_framebuffer_dimensions())
    }
}
...
unsafe impl glium::backend::Backend for GlState {
    ...
    fn get_framebuffer_dimensions(&self) -> (u32, u32) {
        (self.surface.query_width().unwrap() as u32, self.surface.query_height().unwrap() as u32)
    }
