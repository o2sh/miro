    ...
    fn next(state: &Arc<Mutex<Self>>) -> Fallible<()> {
        ...
        if let Some(event) = mouse.pop()? {
            ...
            mouse.future = Some(
                mouse
                    .client
                    .mouse_event(SendMouseEvent { tab_id: mouse.remote_tab_id, event })
                    .then(move |resp| {
...
impl ClientTab {
    pub fn new(client: &Arc<ClientInner>, remote_tab_id: TabId, size: PtySize) -> Self {
        ...
        let writer = TabWriter { client: Arc::clone(client), remote_tab_id };
...
impl Tab for ClientTab {
    ...
    fn send_paste(&self, text: &str) -> Fallible<()> {
        self.client
            .client
            .send_paste(SendPaste { tab_id: self.remote_tab_id, data: text.to_owned() });
        Ok(())
    }
    ...
    fn resize(&self, size: PtySize) -> Fallible<()> {
        ...
        self.client.client.resize(Resize { tab_id: self.remote_tab_id, size });
        Ok(())
    }
    ...
    fn key_down(&self, key: KeyCode, mods: KeyModifiers) -> Fallible<()> {
        self.client.client.key_down(SendKeyDown {
            ...
            event: KeyEvent { key, modifiers: mods },
        });
        Ok(())
    }
    ...
    fn selection_range(&self) -> Option<SelectionRange> {
        *self.renderable.borrow().inner.borrow().selection_range.lock().unwrap()
    }
}
...
impl RenderableInner {
    fn poll(&mut self) -> Fallible<()> {
...

        {
            self.last_poll = Instant::now();
            self.poll_future =
                Some(self.client.client.get_tab_render_changes(GetTabRenderChanges {
                    tab_id: self.remote_tab_id,
...
impl Renderable for RenderableState {
    ...
    fn make_all_lines_dirty(&mut self) {
        self.inner.borrow().something_changed.store(true, Ordering::SeqCst);
    }

    fn clean_dirty_lines(&mut self) {}

    fn current_highlight(&self) -> Option<Arc<Hyperlink>> {
        self.inner.borrow().highlight.lock().unwrap().as_ref().cloned()
    }
...
impl std::io::Write for TabWriter {
    fn write(&mut self, data: &[u8]) -> Result<usize, std::io::Error> {
        self.client
            .client
            .write_to_tab(WriteToTab { tab_id: self.remote_tab_id, data: data.to_vec() })
            .wait()
