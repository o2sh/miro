...
impl Texture {
    fn new<F: Facade>(facade: &F, width: u32, height: u32) -> Result<Self, Error> {
        ...
        Ok(Self { texture, width, height, bottom: 0, tallest: 0, left: 0 })
    }
    ...
    fn reserve<'a, T: Texture2dDataSource<'a>>(
        ...
        let rect = Rect { left: self.left, bottom: self.bottom, width, height };
    ...
    pub fn allocate<'a, F: Facade, T: Texture2dDataSource<'a>>(
        ...
        for tex in self.textures.iter_mut() {
            match tex.reserve(width, height, data) {
                Ok(rect) => {
                    return Ok(Sprite { texture: Rc::clone(&tex.texture), coords: rect });
                }
                Err(dat) => data = dat,
            }
        }
        ...
        let sprite = Sprite { texture: Rc::clone(&tex.texture), coords: rect };
