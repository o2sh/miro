use crate::font::{ftwrap, Font, GlyphInfo};
use crate::pty::MasterPty;
use crate::xgfx::{self, BitmapImage, Connection, Drawable};
use crate::xkeysyms;
use failure::Error;
...
impl<'a> TerminalWindow<'a> {
    ...
    fn cached_glyph(&self, info: &GlyphInfo) -> Result<Rc<CachedGlyph>, Error> {
        let key = GlyphKey { font_idx: info.font_idx, glyph_pos: info.glyph_pos };
            ...
            for (line_idx, line) in dirty_lines {
                ...
                for (cell_idx, info) in glyph_info.iter().enumerate() {
                    ...
                    let (fg_color, bg_color) = if attrs.reverse() {
                        (palette.resolve(&attrs.background), palette.resolve(&attrs.foreground))
                    } else {
                        (palette.resolve(&attrs.foreground), palette.resolve(&attrs.background))
                    };
                    ...
                    self.buffer_image.clear_rect(
                        ...
                        cell_height,
                        if is_cursor_cell { palette.cursor() } else { bg_color }.into(),
                    );
