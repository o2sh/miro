...
impl TerminalState {
    ...
    pub fn visible_cells(&self) -> (usize, &[Line]) {
        let screen = if self.alt_screen_is_active { &self.alt_screen } else { &self.screen };
    ...
    fn parse_sgr(params: &[i64]) -> Option<(CSIAction, &[i64])> {
        if params.len() > 5 {
            ...
            if params[0] == 38 && params[1] == 2 {
                return Some((
                    CSIAction::SetForegroundColor(color::ColorAttribute::Rgb(color::RgbColor {
                        red: params[3] as u8,
            ...
            if params[0] == 48 && params[1] == 2 {
                return Some((
                    CSIAction::SetBackgroundColor(color::ColorAttribute::Rgb(color::RgbColor {
                        red: params[3] as u8,
                        green: params[4] as u8,
                        blue: params[5] as u8,
                    })),
        ...
        match p {
            ...
            30...37 => Some((
                CSIAction::SetForegroundColor(color::ColorAttribute::PaletteIndex(p as u8 - 30)),
                &params[1..],
            )),
            39 => Some((
                CSIAction::SetForegroundColor(color::ColorAttribute::Foreground),
                &params[1..],
            )),
            90...97 => {
                // Bright foreground colors
                Some((
                    CSIAction::SetForegroundColor(color::ColorAttribute::PaletteIndex(
                        p as u8 - 90 + 8,
                    )),
                    &params[1..],
                ))
            ...
            40...47 => Some((
                CSIAction::SetBackgroundColor(color::ColorAttribute::PaletteIndex(p as u8 - 40)),
                &params[1..],
            )),
            49 => Some((
                CSIAction::SetBackgroundColor(color::ColorAttribute::Background),
                &params[1..],
            )),
            100...107 => {
                // Bright background colors
                Some((
                    CSIAction::SetBackgroundColor(color::ColorAttribute::PaletteIndex(
                        p as u8 - 100 + 8,
                    )),
                    &params[1..],
                ))
