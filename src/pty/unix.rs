...
fn openpty(size: PtySize) -> anyhow::Result<(UnixMasterPty, UnixSlavePty)> {
    ...
    let master = UnixMasterPty { fd: PtyFd(unsafe { FileDescriptor::from_raw_fd(master) }) };
    let slave = UnixSlavePty { fd: PtyFd(unsafe { FileDescriptor::from_raw_fd(slave) }) };
...
impl PtyFd {
    ...
    fn spawn_command(&self, mut cmd: Command) -> anyhow::Result<std::process::Child> {
        unsafe {
...
impl SlavePty for UnixSlavePty {
    fn spawn_command(&self, cmd: Command) -> Result<Box<dyn Child + Send>, Error> {
        Ok(Box::new(self.fd.spawn_command(cmd)?))
    }
}
