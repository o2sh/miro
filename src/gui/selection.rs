use crate::core::surface::line::DoubleClickRange;
...
impl SelectionRange {
    ...
    pub fn word_around(start: SelectionCoordinate, renderer: &mut Terminal) -> Self {
        ...
        match lines[0].compute_double_click_range(start.x, is_double_click_word) {
            DoubleClickRange::Range(click_range) => Self {
                start: SelectionCoordinate { x: click_range.start, y: first },
                end: SelectionCoordinate { x: click_range.end - 1, y: first },
            },
            DoubleClickRange::RangeWithWrap(range_start) => {
                let start_coord = SelectionCoordinate { x: range_start.start, y: first };

                let mut end_coord = SelectionCoordinate { x: range_start.end - 1, y: first };
                    ...
                    match lines[0].compute_double_click_range(0, is_double_click_word) {
                        DoubleClickRange::Range(range_end) => {
                            if range_end.end > range_end.start {
                                end_coord = SelectionCoordinate { x: range_end.end - 1, y: y_cont };
                            }
                            break;
                        }
                ...
                Self { start: start_coord, end: end_coord }
            }
        }
    }
